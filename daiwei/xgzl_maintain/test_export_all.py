...
class SjwhTest(StartEnd, BaseView):
    ...
    export_all = (By.XPATH, '//*[@id="showImportId"]')  # 导出全部
    slavepath = r'C:\Users\Ymd\Downloads\工作量数据维护导出.xlsx'  # 文件全路径
    ...
    def test_login_zhaobingcheng(self):
        ...
        try:
            assert NUM == h - 1
            logging.info('新工作量数据维护导出全部成功')
        except:
            logging.info('新工作量数据维护导出全部失败')
            l.getScreenShot('新工作量数据维护导出全部失败')
