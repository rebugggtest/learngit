...
class ZjdTest(StartEnd, BaseView):
    ...
    export_all = (By.XPATH, '/html/body/div[1]/div[2]/div/a[3]')  # 导出全部
    slavepath = r'C:\Users\Ymd\Downloads\中继段管理导出.xlsx'  # 文件全路径
    ...
    def test_login_zhaobingcheng(self):
        ...
        h = sheet.max_row  # 行
        logging.info('导出表数据的行数为%s行' % h)
        time.sleep(2)
        try:
            assert NUM == h - 1
            logging.info('中继段信息管理导出全部成功')
        except:
            logging.info('中继段信息管理导出全部失败')
            l.getScreenShot('中继段信息管理导出全部失败')
