...
class XxjlTest(StartEnd, BaseView):
    ...
    export_current = (By.XPATH, '//*[@id="searchPanel"]/div/div/a[1]')  # 导出当前
    slavepath = r'C:\Users\Ymd\Downloads\导出.xlsx'  # 文件全路径
...
# 导出当前数据
        ...
        try:
            assert NUM == h - 1
            logging.info('巡线记录导出当前成功')
        except:
            logging.info('巡线记录导出当前失败')
            l.getScreenShot('巡线记录导出当前失败')
