...
class RsjlrTest(StartEnd, BaseView):
    ...
    export_current = (By.XPATH, '//*[@id="tool"]/div/div/a[3]')  # 导出当前
    slavepath = r'C:\Users\Ymd\Downloads\人手井信息导出.xlsx'  # 文件全路径
...
# 导出当前数据
        ...
        try:
            assert NUM == h - 1
            logging.info('人手井录入导出当前成功')
        except:
            logging.info('人手井录入导出当前失败')
            l.getScreenShot('人手井录入导出当前失败')
