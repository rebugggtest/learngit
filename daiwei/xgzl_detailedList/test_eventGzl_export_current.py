...
# 运维管理 - 工作量管理（新） - 新工作量详单 - 事件工作量查询 - 导出当前
...
class XdTest(StartEnd, BaseView):
    ...
    select = (By.XPATH, '/html/body/div[2]/div[1]/div[2]/div/div[1]/div[1]/div[1]/div/table/tbody/tr/td[2]/div/input')  # 选择当前页数据
    export_current = (By.XPATH, '/html/body/div[2]/div[1]/div[1]/div/a[1]')  # 导出当前
    ...
    # 导出当前数据
        ...
        try:
            assert NUM == h - 1
            logging.info('事件工作量查询导出当前成功')
        except:
            logging.info('事件工作量查询导出当前失败')
            l.getScreenShot('事件工作量查询导出当前失败')
