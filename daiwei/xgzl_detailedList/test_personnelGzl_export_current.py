...
# 运维管理 - 工作量管理（新） - 新工作量详单 - 人员工作量查询 - 导出当前
...
class XdTest(StartEnd, BaseView):
    ...
    personnelGzl = (By.XPATH, '//*[@id="s2"]')  # 人员工作量查询
    resetBtn = (By.XPATH, '//*[@id="clean"]')  # 重置
    string = (By.XPATH, '/html/body/div[2]/div[2]/div[2]/div/div[2]/div[1]')  # 右下角该模块数据条数
    select = (By.XPATH, '/html/body/div[2]/div[2]/div[2]/div/div[1]/div[1]/div[1]/div/table/tbody/tr/td[2]/div/input')  # 选择当前页数据
    export_current = (By.XPATH, '/html/body/div[2]/div[2]/div[1]/div/a[1]')  # 导出当前
    ...
    def test_login_zhaobingcheng(self):
        ...
        self.driver.find_element(*self.personnelGzl).click()
        time.sleep(1)
    ...
    # 导出当前数据
        ...
        try:
            assert NUM == h - 1
            logging.info('人员工作量查询导出当前成功')
        except:
            logging.info('人员工作量查询导出当前失败')
            l.getScreenShot('人员工作量查询导出当前失败')
