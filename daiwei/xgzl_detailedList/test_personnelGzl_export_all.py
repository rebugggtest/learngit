...
# 运维管理 - 工作量管理（新） - 新工作量详单 - 人员工作量查询 - 导出全部
...
class XdTest(StartEnd, BaseView):
    ...
    export_all = (By.XPATH, '/html/body/div[2]/div[2]/div[1]/div/a[2]')  # 导出全部
...
# 获取该模块数据条数
        ...
        try:
            assert NUM == h - 1
            logging.info('人员工作量查询导出全部成功')
        except:
            logging.info('人员工作量查询导出全部失败')
            l.getScreenShot('人员工作量查询导出全部失败')
