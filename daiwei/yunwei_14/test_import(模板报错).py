...
class GldTest(StartEnd, BaseView):
    ...
    slavepath = r'C:\Users\Ymd\Downloads\杆路段管理导入模板.xlsx'  # 文件全路径
    file_select = (By.XPATH, '//*[@id="FormUpdate"]/div/span/a/label')  # 点击选择文件
    submitBtn = (By.XPATH, '//*[@id="win"]/div[3]/a[1]/span/span')  # 提交
    close_alert = (By.XPATH, '/html/body/div[20]/div[1]/div[2]')  # 关闭提示信息
    resetBtn = (By.XPATH, '//*[@id="searchBox"]/ul[2]/li[3]/a[2]')  # 重置
    deleteBtn = (By.XPATH, '//*[@id="datagrid-row-r1-2-0"]/td[18]/div/a[2]')  # 删除
    ...
    def test_login_zhaobingcheng(self):
        ...
        sheet['B2'] = self.excel_start_type
        sheet['C2'] = self.excel_start_name
        sheet['D2'] = self.excel_end_type
        sheet['E2'] = self.excel_end_name
        sheet['I2'] = self.excel_share_unit
        sheet['J2'] = self.excel_co_unit
        sheet['N2'] = self.excel_province
        sheet['O2'] = self.excel_city
        sheet['P2'] = self.excel_county
        ...
        try:
            ...
            logging.info('杆路段信息管理导入成功')
        except:
            logging.info('杆路段信息管理导入失败')
            l.getScreenShot('杆路段信息管理导入失败')
        ...
        WebDriverWait(self.driver, 5).until(lambda x: x.find_element(*self.resetBtn))
        self.driver.find_element(*self.resetBtn).click()
        time.sleep(5)
